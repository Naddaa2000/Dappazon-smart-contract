...
const Product = ({ item, provider, account, dappazon, togglePop }) => {
    ...
    const [owner, setOwner] = useState("") // Define owner state variable
    const [isOwner, setIsOwner] = useState(false) // State to track if connected account is owner

    const [hasBought, setHasBought] = useState(false)
    const [ownerAmount, setOwnerAmount] = useState(null)
    ...
    const buyHandler = async () => {
        ...
        // await transaction.wait()
        // const updatedOwnerAmount = await provider.getBalance(owner)

        // // Update owner's amount state
        // setOwnerAmount(updatedOwnerAmount)
